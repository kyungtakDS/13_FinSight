...
4. **RLS 확인:** Table Editor에서 6개 테이블 모두 RLS가 켜져 있는지 눈으로 확인. 꺼져 있으면 배포하지 마라.
...
- [ ] 회원가입 → 이메일 인증 → 로그인
- [ ] 샘플 CSV 업로드 → 거래 건수와 분류 결과 확인
- [ ] 같은 파일 재업로드 → 중복 0건 확인
- [ ] 카테고리 수정 → 새로고침 후 유지되는지
- [ ] Free 상태에서 구독 목록·인사이트가 잠겨 있는지
...
- 외부 전송: 가맹점명(유니크 목록)과 집계 결과가 Anthropic Claude API로 전송됨. 개별 거래 원문은 전송하지 않음.
