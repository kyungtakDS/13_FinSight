...
| **B** | **가맹점→카테고리 맵을 쓴다** (테이블이 아니라 `transactions` 파생 쿼리) | 매달 같은 가맹점이 다시 미분류로 들어와 LLM에 재전송된다. 맵이 있으면 2회차부터 *새 가맹점만* 보내고, "한 번 고치면 전부 반영"과 "수정이 다음 달에도 유지"가 따라온다. 처음에는 전용 테이블로 설계했다가 **최종 점검에서 쿼리로 바꿨다** — 그 정보는 이미 `transactions.category`에 있었다(ADR-009·013) |
...
| **DE-11** | 2회차 업로드 | 지난달에 분류한 가맹점을 또 LLM에 보냄 | step 7 — 과거 거래에서 파생한 가맹점 맵 |
