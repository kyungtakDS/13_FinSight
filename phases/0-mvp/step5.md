...
**이 필터는 "무엇을 보여줄지"에만 적용한다. "무엇을 계산할지"에는 적용하지 마라.**

- `categoryBreakdown` / `periodTrend` → 보관 기간 필터를 거친 거래로 계산한다 (Free는 최근 3개월치만 보인다)
- `detectRecurring` / `detectAnomalies` → **전체 이력으로 계산한다**

이유: 반복 결제는 **3회 이상**이라야 탐지되는데, 월간 구독이 3회 쌓이려면 최소 90일이 필요하다. 필터를 먼저 걸면 Free 사용자는 경계선에 딱 걸려 반복 결제가 거의 잡히지 않고, ADR-010이 Free에 보여주기로 한 "반복 결제 12건, 월 ₩84,300"이 대부분 0건이 된다. 전환 근거로 쓰려던 수치가 사라지는 것이다.

사용자 자신의 데이터를 근거로 한 집계이므로 전체 이력으로 세는 것이 부당하지 않다. 잠그는 것은 **목록**이지 계산이 아니다.
