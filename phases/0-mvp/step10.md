...
- **일반 절차 하나만 쓴다:** "카드사 홈페이지 로그인 → 이용내역조회 → 기간 선택 → 엑셀/CSV 내려받기". 카드사 이름은 나열하되 **카드사별 클릭 순서를 지어내지 마라** — 확인할 수 없고, 틀린 안내는 없는 안내보다 나쁘며, 사이트가 개편되면 바로 낡는다. 실제 문의가 쌓이면 그때 자주 막히는 카드사부터 추가한다.
