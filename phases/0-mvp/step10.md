...
**개인정보** — 짧고 정직하게. `"거래 내역은 계정별로 격리되어 저장되며, 카드·계좌·주민등록번호는 저장 전에 마스킹됩니다. 분석을 위해 가맹점명이 Anthropic Claude API로 전송되며, 계좌이체처럼 사람 이름이 들어가는 거래는 전송하지 않습니다."`
...
- **한계 명시** (`docs/PRD.md`의 "알려진 파싱 한계" 그대로): 카드별 구분 미지원, 할부 이중 계상 가능성, 부분취소 자동 상계 안 함. 기대를 만들어놓고 배신하는 것보다 낫다.
