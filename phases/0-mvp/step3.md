...
`docs/ARCHITECTURE.md`의 데이터 모델 표대로 6개 테이블을 만든다: `profiles`, `subscriptions`, `statements`, `transactions`, `recurring_charges`, `insights`.
...
- `alter table <t> enable row level security;` × 6
...
npm run build
npm run test
grep -c "enable row level security" supabase/migrations/0002_rls.sql   # 6 이상이어야 한다
```

## 검증 절차

1. 위 AC 커맨드를 실행한다. `grep` 결과가 6 미만이면 실패로 간주하고 정책을 보완하라.
2. 아키텍처 체크리스트:
   - 6개 테이블 전부 RLS가 켜져 있고 정책이 있는가?
